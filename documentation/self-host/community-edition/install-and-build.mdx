...
10. `MAILER_SMTP_ENABLE`: Enables the SMTP mailer configuration.
11. `MAILER_USE_CUSTOM_CONFIGS`: When custom mailer configurations are used.
12. `MAILER_SMTP_URL`: The SMTP URL for email delivery.
13. `MAILER_ADDRESS_FROM`: The email address that you would be using.
14. `MAILER_SMTP_HOST`: The SMTP host.
15. `MAILER_SMTP_PORT`: The port to connect to the SMTP server.
16. `MAILER_SMTP_USER`: The SMTP user or email for authentication.
17. `MAILER_SMTP_PASSWORD`: Provide the password set for the SMTP user.
18. `RATE_LIMIT_TTL`: The time it takes to refresh the maximum number of requests being received.
19. `RATE_LIMIT_MAX`: The maximum number of requests that Hoppscotch can handle under `RATE_LIMIT_TTL`.
20. `VITE_BASE_URL`: This is the URL where your deployment will be accessible from.
21. `VITE_SHORTCODE_BASE_URL`: A URL to generate shortcodes for sharing, can be the same as `VITE_BASE_URL`.
22. `VITE_BACKEND_GQL_URL`: The URL for GraphQL within the instance.
23. `VITE_BACKEND_WS_URL`: The URL for WebSockets within the instance.
24. `VITE_BACKEND_API_URL`: The URL for REST APIs within the instance.
25. `VITE_APP_TOS_LINK` and `VITE_APP_PRIVACY_POLICY_LINK` are optional and are used to configure the links to the Terms & Conditions and Privacy Policy.
