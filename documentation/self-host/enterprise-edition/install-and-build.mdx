...
ENABLE_HORIZONTALLY_SCALING=false

# Redis Config 
# Note: Configure Redis only if ENABLE_HORIZONTALLY_SCALING is set to true
...
3. `ENABLE_HORIZONTALLY_SCALING`: Set to true to enable horizontal scaling, which uses Redis for managing sessions and state across instances.
