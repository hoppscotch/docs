...
Hoppscotch considers an object within angular brackets as a variable, e.g. `<<variable>>`.
...
The scope of each variable can be identified from the color of the variable name. If a variable with the same name exists, it is resolved in the order of priority.
