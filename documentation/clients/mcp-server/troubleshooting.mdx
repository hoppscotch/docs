...
**Reason:** The server detected a continuous integration runner, an SSH session, or a machine with no display. Device login cannot complete there, so it fails immediately instead of waiting out `HOPPSCOTCH_AUTH_TIMEOUT_MS` (60 seconds by default) for a browser that never opens.
...
**Fix:** Run the `reauth` tool, which discards the stored session and opens a fresh browser sign-in, or restart the server.
...
**Reason:** Cloud's `userCollection` resolver fails to serialize the `data` field, which fails the whole query, so the server refuses the call up front.
...
## `search_team_requests` on Hoppscotch Cloud
...
**Fix:** Once `HOPPSCOTCH_AUTH_TIMEOUT_MS` (60 seconds by default) elapses, the tool result itself returns the login URL; it is also written to the server's error output. Open it manually, then retry the call.
