...
### Running Collections and Environments on Cloud

You can now connect your Hoppscotch Cloud account with the CLI using Persoanl Access Token and execute collections and environments directly by specifying their respective **IDs** in the command. It's important to note that Hoppscotch Cloud functions as the default server in this scenario.

```bash
hopp test  [-e <environment id>] [-d <delay_i_ms>] <hoppscotch collection id> [--token <access_token>]
```

### Running Collections and Environments on Self Hosted instance

To establish a connection with self-hosted Hoppscotch instances, you'll need to specify the server URL along with the access token and IDs to run collections and environments on CLI.
...
<Tip> You can directly copy the command with the auto-populated **Collection ID** by navigating to the **“CLI"** tab within the **“Run Collection”** action found in the context menu. </Tip>
...
- `delay_i_ms` : Represnts a time interval (in milliseconds) to pause execution of API requests before within a collection.
- `access token` : It is a secure, unique identifier used to authenticate a user's access to their Hoppscotch account and its resources like collections, environements, requests and other entities. [Learn more about retrieving your PAT by checking our documentation](/documentation/features/pat)
- `server url` : This is the address where your self-hosted Hoppscotch instance is running.
...
To run an environment using its ID, click on the `Properties` action located in the menu icon next to each environment. Within the Details section, you'll find the **Environment ID**. Copy this ID and use it in the Hoppscotch CLI for execution.
...
|`--token`        | Expects a PAT value to be passed for establishing connection with Hoppscotch account.                             |
|`--server`       | Demands a server URL where self-hosted instance is running.                                                       |
