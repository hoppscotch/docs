...
```bash
hopp test  [-e <environment id>] [-d <delay_in_ms>] <hoppscotch collection id> [--token <access_token>] [--server <server url>]
```
...
- `delay_in_ms` : Represents a time interval (in milliseconds) to pause execution of API requests before within a collection.
...
- `server url` : This is optional and is the URL of your self-hosted instance when you're self-hosting your API client
