...
| `name`	                      | Attribute for For `testsuite`, it indicates the hierarchy of collections up to the request. Organized at the request level using the naming convention: `<parent-collection-name>/<child-collection-name>/<request-name>`. While, for `testcase`, it'll be the description.                                             
