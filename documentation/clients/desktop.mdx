...
<Note>
  e.g. to allow connection to `https://hoppscotch.mydomain.com` you need to add `app://hoppscotch_mydomain_com` and `http://app.hoppscotch_mydomain_com` to the `WHITELISTED_ORIGINS` environment variable.
  ```bash
  WHITELISTED_ORIGINS=...existing_origins,app://hoppscotch_mydomain_com,http://app.hoppscotch_mydomain_com
  ```
  <Tip>
    _app://hoppscotch_mydomain_com_ &nbsp; for Linux and macOS machines.
  </Tip>
  <Tip>
    _http://app.hoppscotch_mydomain_com_ &nbsp; for Windows machine.
  </Tip>
</Note>
