...
<Note>
  ...
  <p>
    e.g. to allow connection to `https://hoppscotch.acme.com` you need to add `app://hoppscotch_acme_com` to the `WHITELISTED_ORIGINS` environment variable.
    ```bash
    WHITELISTED_ORIGINS=...existing_origins,app://hoppscotch_acme_com
    ```
  </p>
</Note>
