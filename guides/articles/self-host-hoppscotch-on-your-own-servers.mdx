...
Self-hosted Hoppscotch comes in two variants - [Community](https://docs.hoppscotch.io/documentation/self-host/community-edition/getting-started) and [Enterprise Edition](https://docs.hoppscotch.io/documentation/self-host/enterprise-edition/getting-started), both of which can be deployed on systems that support Docker. You can host Hoppscotch on your servers, providing a workspace that is private to the individuals or teams using Hoppscotch.

This guide covers the basics of self-hosting Hoppscotch, including the necessary configurations and settings you need to get started.

## Pre-requisites

Before you start self-hosting Hoppscotch, ensure that your system or environment meets the following requirements:
...
By default, Hoppscotch ships with a Docker container that has a pre-configured Postgres database. If you prefer to use your own Postgres database, either on the cloud or locally, you’ll need to set up a valid database URL with the correct credentials.
...
To enable subpath-based access, set `ENABLE_SUBPATH_BASED_ACCESS=true` (the **default** setting is **false**).

When enabled, here's how it affects your setup:

- **Individual Containers**: You must configure a reverse proxy to route requests to the appropriate containers based on their subpaths.
- **AIO Container**: The services can be accessed through specific routes:
    - Hoppscotch App: `/`
